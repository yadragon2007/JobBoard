<x-app-layout>
    ...
    <div class="overflow-x-auto p-6">
        <div class="max-w-xl mx-auto p-6 bg-white rounded-lg shadow">
            ...
            <x-previous-page-arrow :default="route('job-vacancy.show',$jobVacancy->id) " :comeBack="false"/>   
            ...
            <form action="{{ route("job-vacancy.update", $jobVacancy->id) }}" method="post">
                    ...
                    <div class="flex-1 p-4 rounded-xl ">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="employment_type" name="employment_type"
                                ...
                                <option value="{{ null }}" {{!old("employment_type") ? "selected" : ""}}>Select Employment type</option>
                                <option value="Full-time" {{(old("employment_type") == "Full-time" && old("employment_type") != null) || $jobVacancy->employment_type == "Full-time" ? "selected" : ""}}>Full-time</option>
                                <option value="Part-time" {{(old("employment_type") == "Part-time" && old("employment_type") != null) || $jobVacancy->employment_type == "Part-time" ? "selected" : ""}}>Part-time</option>
                                <option value="Contract" {{(old("employment_type") == "Contract" && old("employment_type") != null) || $jobVacancy->employment_type == "Contract" ? "selected" : ""}}>Contract</option>
                                <option value="Remote" {{(old("employment_type") == "Remote" && old("employment_type") != null) || $jobVacancy->employment_type == "Remote" ? "selected" : ""}}>Remote</option>
                                <option value="Hybrid" {{(old("employment_type") == "Hybrid" && old("employment_type") != null) || $jobVacancy->employment_type == "Hybrid" ? "selected" : ""}}>Hybrid</option>
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="company_id" name="company_id" 
                                ...
                                <option value="{{ null }}" {{!old("company_id") ? "selected" : ""}}>Select a Company</option>
                                @forelse($companies as $company)
                                     <option value="{{ $company->id }}" {{(old("company_id") == $company->id && old("company_id") != null) || $jobVacancy->company_id == $company->id ? "selected" : ""}}>
                                        {{$company->name}}
                                    </option>                               
                                     @empty
                        ...
                        <div class="mb-4">
                            ...
                            <select id="job_category_id" name="job_category_id"
                                ...
                                <option {{!old("category_id") ? "selected" : ""}}>Select Job Category</option>


                                @forelse($categories as $category)
                                    <option value="{{ $category->id }}" {{(old("job_category_id") == $category->id && old("job_category_id") != null) || $jobVacancy->job_category_id == $category->id ? "selected" : ""}}>
                                        {{$category->name}}
                                    </option>
                                @empty
                ...
                <div class="flex justify-end items-center space-x-6 mt-2">
                    <x-previous-page-cancel :default="route('job-vacancy.show', $jobVacancy->id)" :comeBack="false" />
