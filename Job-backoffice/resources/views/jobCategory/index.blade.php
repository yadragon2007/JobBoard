    ...
    <div class="overflow-x-auto p-6">
        ...
        <div class="w-full flex justify-end items-center space-x-4 pb-1">
            ...
            @if (request()->has("archived") && request("archived") == "true")
                <a href="{{ route('job-category.index', ["archived" => "false"]) }}"
            ...
            <tbody class="divide-y">
                ...
                @forelse ($categories as $category)
                    <tr>
                        ...
                        <td class="px-6 py-3 text-center text-sm font-semibold text-gray-800">{{ $category->name }}</td>
                        <td class="px-6 py-3 text-center text-sm font-semibold text-gray-800 flex items-center">
                            ...
                            @if (request()->has("archived") && request("archived") == "true")
                                ...
                                <form class="flex-1 text-red-500 hover:text-red-800"
                                    action="{{ route("job-category.restore", $category->id) }}" method="post">
                                    @csrf
                                    @method('put')
                            ...
                            @else
                                ...
                                <!-- edit -->
                                <a class="flex-1 text-blue-500 over:text-blue-800"
                                    href="{{ route("job-category.edit", $category->id) }}">Edit</a>
                                <!-- archived -->
                                <form class="flex-1 text-red-500 hover:text-red-800"
                                    action="{{ route("job-category.destroy", $category->id) }}" method="post">
                                    @csrf
                                    @method('DELETE')
