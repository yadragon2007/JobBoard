@php
    $UpdateCompanyFormAction = auth()->user()->role === "admin" ? route("company.update", $company->id) : route("my-company.update");
@endphp
...
<x-app-layout>
    ...
    <div class="overflow-x-auto p-6">
        <div class="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow">
            <x-previous-page-arrow :default="route('company.show', $company->id)" :comeBack="false" />

            <form action="{{ $UpdateCompanyFormAction }}" method="post">
                ...
                <div class="flex justify-between space-x-4">
                    ...
                    <div class="flex-1 p-4 rounded-xl ">
                        ...
                        <div class="mb-4">
                            <label for="ownerName" class="block text-sm font-medium text-gray-700">Owner
                                Name</label>
                            <input type="text" name="ownerName" id="ownerName"
                                value="{{ old('email', $company->owner->name)}}"
                        ...
                        <!-- Owner Email read only (can not be changed) -->
                        <div class="mb-4">
                            <label for="email" class="block text-sm font-medium text-gray-700">Email</label>
                            <input disabled type="email" name="email" id="email"
                                value="{{ old('email', $company->owner->email) }}"
                                class="{{ $errors->has('email') ? 'outline-1 outline-red-500 outline' : ''}} bg-gray-100  mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            @error('email')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="flex justify-end items-center space-x-6 mt-2">
                    <x-previous-page-cancel :default="route('company.show', $company->id)" :comeBack="false" />
