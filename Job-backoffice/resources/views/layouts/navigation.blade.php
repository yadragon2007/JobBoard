<nav class="w-[250px] h-screen bg-white border-r border-gray-200">
    ...
    <div class="flex items-center px-6 border-b border-gray-200 py-4">
        <a href="{{ route("dashboard.index") }}" class="text-2xl font-bold flex items-center space-x-2">
    ...
    <ul class="flex flex-col px-4 py-6 space-y-2">
        ...
        <form action="{{ route('logout') }}" method="post">
            @csrf
            <!-- Using utilities: -->
            <button type="submit" class="bg-red-500 w-full hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                Logout
            </button>
        </form>
    </ul>
